...
export default defineEventHandler(async (event) => {
  ...
  if (!category || !['receipts', 'expenses'].includes(category)) {
    throw createError({ statusCode: 400, statusMessage: 'category phải là receipts hoặc expenses' })
  }

  const entityName = (query.entity_name as string) || 'upload'
  const entityId = (query.entity_id as string) || `temp-${Date.now()}`
  ...
  // {category}/{year}/{month}/{id-name}
  const folderId = await getOrganizedFolderId(category, entityId, entityName)
